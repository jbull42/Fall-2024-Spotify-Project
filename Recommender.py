...
def get_song_id(token,song_name):
    ...
    if not tracks:
        print("No songs with this name found.")
        return None
    song_id = tracks[0]["id"]
    return song_id
...
def get_album_id(token,album_name):
    ...
    if not albums:
        print("No albums with this name found")
        return None
...
def get_song_data(token, song):
    url = f'https://api.spotify.com/v1/audio-features/{get_song_id(token, song)}'
    print(url)
    headers = get_auth_header(token)
    result = get(url, headers=headers)
    json_result = json.loads(result.content)
...
#     if not song_id:
#         print(f"Error: No song found with the name {song_name}")
#         return None
...
#         print(f"Error fetching song data: {result.status_code}")
#         return None
...
#     if selected_song:
#         if get_song_data(token, selected_song):
#             data = get_song_data(token, selected_song)
#             list_genres = get_genres_based_on_song(token, selected_song)
#             new_list = []
#             for i in range (min(len(list_genres),5)):
#                 new_list.append(list_genres[i])
#             songs = several_browse_categories(token, genres, int(data['speechiness']*100),data['duration_ms']/60000.0,(data['loudness'] + 60) * (100 / 60),int(data['energy'] * 100),int(data['valence'] * 100))
#             if songs:
#                 st.header("**Recommended Songs**")
#                 for i, song in enumerate(songs):
